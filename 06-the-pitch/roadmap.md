...
**Now:**
Our strategy isn't "use AI to move faster" in the abstract anymore — it's Doc, a confirm-gated Oracle embedded in the GP's existing chart-open moment, defensible for now through workflow depth while the correction-capture loop turns that into a real data moat against iCURE. The AI-adjusted economics are known and worse than the M1 framing implied (75% → 60% margin) but still net-positive at $26.45/GP/month, and every suggestion sits inside EU AI Act high-risk and GDPR Art. 9 controls rather than the earlier "it just needs to work" framing that never named a compliance surface. What hasn't changed since M1: we still don't have a chosen pricing model or a defined reliability target — those are the two gaps this funding round has to close before Doc scales past the pilot.
